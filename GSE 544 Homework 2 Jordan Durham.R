...
# Read in temperature data and only keep relevant columns

temp = read.csv("Temperature.csv")
temp[which(!(names(temp) %in% c("Year", "Use")))] = NULL

# Prepare CO2 datasets for merging
...
# Combine the the unshared and shared years of CO2 data
...
train = cbind(train_co2[order(match(train_temp$Year, train_co2$year)),], Temp = train_temp$Use)
